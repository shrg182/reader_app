...
def index():
    ...
    progress_values = {
        book.id: json.loads(book.progress.locator).get("progress", 0) if book.progress else 0
        for book in books
    }
    if sort == "title":
...
def import_book():
    if request.method == "POST":
        ...
        if uploaded is None or not uploaded.filename:
            flash("Choose a TXT or Markdown file.", "error")
        ...
        uploaded.save(Path(current_app.config["BOOK_UPLOAD_FOLDER"]) / stored_name)
